...
save_dir = "results/%s_gen/%s_lr%.3f_seed%d_scale%.1f_nsteps%d" % (
    args.mode,
...
if args.use_muon:
    ...
    # ]
    im_optimizer = Muon([im], lr=args.lr, momentum=0.95, nesterov=True)
else:
    im_optimizer = torch.optim.AdamW([im], lr=args.lr, betas=(0.9, 0.99), eps=1e-15) # [1, 4, 64, 64]
if args.mode == "vsd":
...
for step in tqdm(range(args.n_steps)):
    ...
    im_optimizer.step()
    im_optimizer.zero_grad(set_to_none=True)
