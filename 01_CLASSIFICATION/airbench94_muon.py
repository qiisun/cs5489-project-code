...
# 增加 train_loss 和 val_loss 列
logging_columns_list = ["run   ", "epoch", "train_acc", "train_loss", "val_acc", "val_loss", "tta_val_acc", "time_seconds"]
...
def print_training_details(variables, is_final_entry):
    ...
    for col in logging_columns_list:
        ...
        if type(var) in (int, str):
            ...
            res = "" if var is None else str(var)
        formatted.append(res.rjust(len(col)))
    print_columns(formatted, is_final_entry=is_final_entry)
...
def evaluate(model, loader, tta_level=0):
    logits = infer(model, loader, tta_level)
    acc = (logits.argmax(1) == loader.labels).float().mean().item()
...
def main(run, model):
    ...
    for epoch in range(ceil(total_train_steps / len(train_loader))):
        ...
        val_acc, val_loss = evaluate(model, test_loader, tta_level=0)

        variables = locals()
        variables["train_loss"] = train_loss
        print_training_details(variables, is_final_entry=False)
        train_logs.append(dict(run=run, epoch=epoch, train_acc=train_acc, train_loss=train_loss, val_acc=val_acc, val_loss=val_loss))

    start_timer()
    tta_val_acc, _ = evaluate(model, test_loader, tta_level=2)
    stop_timer()
    epoch = "eval"
    variables = locals()
    variables["train_loss"] = ""
    variables["val_loss"] = ""
    print_training_details(variables, is_final_entry=True)

    return tta_val_acc, train_logs
...
if __name__ == "__main__":
    ...
    torch.save(dict(code=code, accs=accs, logs=all_logs), log_path)
